...
export function apply(root = document) {
    ...
    root.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        const skipTextareaValue = tag === 'TEXTAREA' && (attrs.includes('placeholder') || attrs.includes('value'));
        if (tag === 'TEXTAREA' && !skipTextareaValue) el.value = val; else if (tag !== 'TEXTAREA') el.textContent = val;
        attrs.forEach(attr => {
